    ...
    <div className="min-h-screen bg-[#071b59] text-white" style={gridStyle}>
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-6 py-8">
        ...
        <section className="grid gap-6 lg:grid-cols-[1fr_1.4fr]">
          <Card className="rounded-none border-white/20 bg-[#082064]/85 text-white"><CardHeader><CardTitle className="flex items-center gap-2 text-sm uppercase tracking-[.18em]"><LockKeyhole size={16} /> Tenant scope</CardTitle></CardHeader><CardContent className="space-y-3"><select aria-label="Tenant scope" value={tenantKey ?? ""} onChange={(event) => setSelectedTenant(event.target.value || null)} className="w-full border border-white/20 bg-[#06184f] px-3 py-3 text-sm text-white outline-none focus:border-cyan-200" disabled={!tenantsQuery.data?.length || tenantsQuery.isError}><option value="">{tenantsQuery.isLoading ? "Cargando scopes..." : "Selecciona un tenant"}</option>{tenantsQuery.data?.map((tenant) => <option key={tenant.tenantKey} value={tenant.tenantKey}>{tenant.name} · {tenant.tier}</option>)}</select>{tenantsQuery.isError ? <p className="text-sm text-amber-200">No se pudieron cargar los tenants de este operador.</p> : selected ? <div className="grid grid-cols-2 gap-3 text-xs"><div className="border border-white/10 p-3"><p className="text-blue-200/60">KEY</p><p className="mt-1 font-mono">{selected.tenantKey}</p></div><div className="border border-white/10 p-3"><p className="text-blue-200/60">STATUS</p><p className="mt-1 text-emerald-200">{selected.status}</p></div></div> : <p className="text-sm text-blue-100/60">No hay tenants visibles en este scope.</p>}</CardContent></Card>

          <Card className="rounded-none border-white/20 bg-[#082064]/85 text-white"><CardHeader><CardTitle className="flex items-center gap-2 text-sm uppercase tracking-[.18em]"><Activity size={16} /> Validation ledger</CardTitle></CardHeader><CardContent>{overviewQuery.isError ? <div className="flex items-center gap-3 py-8 text-sm text-amber-200"><TriangleAlert size={18} /> No se pudo consultar el ledger de este tenant.</div> : overviewQuery.isLoading ? <div className="py-8 text-sm text-blue-100/60">Sincronizando validaciones...</div> : overviewQuery.data?.validations?.length ? <div className="space-y-2">{overviewQuery.data.validations.slice(0, 6).map((validation) => <div key={validation.validationId} className="flex items-center justify-between gap-3 border border-white/10 px-3 py-3 text-xs"><div className="min-w-0"><p className="truncate font-mono text-blue-100">{validation.validationId}</p><p className="mt-1 text-blue-200/55">{validation.artifactType} · {validation.riskClass}</p></div><Badge className="rounded-none border border-white/20 bg-transparent text-[10px] text-cyan-100">{validation.status}</Badge></div>)}</div> : <div className="flex items-center gap-3 py-8 text-sm text-blue-100/60"><Database size={18} /> Aún no existen ejecuciones para este tenant.</div>}</CardContent></Card>
        </section>
